...
ですが、ECSコンテナのデフォルトの設定のヘルスチェックでは③の時点でhealthyとみなすようになっています。
...
ECS
ECSのタスク定義からヘルスチェックエンドポイントを設定します。

```json
      "healthCheck": {
        "command": [
          "CMD-SHELL",
          "curl -f http://localhost:8080/acutuator/health || exit 1"
        ],
        "interval": 30,
        "timeout": 5,
        "retries": 3,
...
`command`には先ほど実装したAcutuatorのエンドポイントを指定しましょう。
