---
title: "Raspberry Pi で監視用のベビーモニターを自作する"
emoji: "👶🏻"
...
---
# はじめに
Raspberry Pi（以下ラズパイと呼ぶ）を使ってベビーモニターを自作しました。
...
Pi 5が最新のモデルですが、今回の自分の用途にはハイスペックすぎるで高価だったので、1ランク下のRasberry Pi 4 Model Bを購入しました。

価格はAmazonで¥9,275でした (2024/06時点)
### カメラモジュール
ラズパイ本体にカメラは付いてないので、追加でカメラモジュールを用意する必要があります。
カメラモジュールも色々と種類がありますが、今回は暗い部屋の中でも撮影したかったので暗視撮影のできるRaspberry Pi カメラモジュール V3（NoIR）を購入しました。
https://www.switch-science.com/products/8701

### 赤外線LEDライト
暗闇で撮影するには暗視カメラと赤外線LEDライトが必要です。
これは、被写体を赤外線光で照射しないとカメラが被写体を認識できないためです。

こちらを購入しました。
https://www.amazon.co.jp/dp/B07GRLQ9TW?psc=1&ref=ppx_yo2ov_dt_b_product_details
...
- Locale
    - Asia/Tokyo
    - jp
![](https://storage.googleapis.com/zenn-user-upload/302dbce21dec-20240623.png)

- SSH
    - Enable SSHにチェック
    - Use password authenticationにチェック
![](https://storage.googleapis.com/zenn-user-upload/2e5700e5ca84-20240623.png)
...
マウス・キーボード・HDMIケーブルもこのタイミングで挿しこむ。
...
公式によると5分以内に起動するとのこと。それ以上時間がかかる場合は何かしらのエラーが起きてるとみてよさそうです。
