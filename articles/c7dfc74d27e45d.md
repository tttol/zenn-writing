...
`mergectl`では、gitリポジトリをクローンしたディレクトリで`mergectl v1 v2`と実行するだけで、v1 -> v2へのマージを実施します。

![v1tov2](/images/v1tov2.png)

また、hotfixブランチなどでmainブランチに変更が入った場合も、同様に`mergectl main v1 v2`でv1. v2ブランチに変更を反映します。
