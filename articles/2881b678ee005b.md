...
なぜNeovim 0.12.2にアップデートするとnvim-treesitterがエラーを吐くようになったのでしょうか。それは、treesitterの機能の一部がプラグインからNeovimのコア機能へと移行したからです。
...
これまでnvim-treesitter自身がシンタックスハイライトなどの機能を担っていましたが、0.12系からはNeovimのコア機能としてハイライトなどが実行されます。nvim-treesitterの主な役割はparser/queryのインストール管理へと縮小します。
...
また、0.12系ではparser/queryのビルドにtree-sitter CLIというツールが必要となりました。これはRust製のツールなので、cargo installでインストールする必要があります。
